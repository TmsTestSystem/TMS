...
router.get('/test-plan/:testPlanId', async (req, res) => {
    try {
...
router.post('/', async (req, res) => {
    try {
        const { testPlanId, name, description } = req.body;
        if (!testPlanId || !name) {
            return res.status(400).json({ error: 'ID тест-плана и название обязательны' });
        }
        const existing = await (0, database_1.query)('SELECT id FROM test_runs WHERE test_plan_id = $1 AND name = $2', [testPlanId, name]);
        ...
        const result = await (0, database_1.query)(`INSERT INTO test_runs (
        test_plan_id, name, description, status, started_by, created_at
      ) VALUES ($1, $2, $3, $4, $5, NOW()) RETURNING *`, [testPlanId, name, description, 'planned', '00000000-0000-0000-0000-000000000001']);
        const testRun = result.rows[0];
        console.log('[CREATE TEST RUN] created test_run_id:', testRun.id);
        const testCasesResult = await (0, database_1.query)('SELECT id FROM test_cases WHERE test_plan_id = $1', [testPlanId]);
        ...
        for (const testCase of testCasesResult.rows) {
            ...
            await (0, database_1.query)(`INSERT INTO test_results (
          test_run_id, test_case_id, status, created_at
        ) VALUES ($1, $2, $3, NOW())`, [testRun.id, testCase.id, 'not_run']);
        }
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const result = await (0, database_1.query)(`UPDATE test_runs 
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await (0, database_1.query)('UPDATE test_runs SET is_deleted = TRUE, deleted_at = NOW() WHERE id = $1 AND is_deleted = FALSE RETURNING *', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Тестовый прогон не найден' });
        }
...
router.post('/:id/restore', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await (0, database_1.query)('UPDATE test_runs SET is_deleted = FALSE, deleted_at = NULL WHERE id = $1 AND is_deleted = TRUE RETURNING *', [id]);
        if (result.rows.length === 0) {
...
router.put('/:id/results/:testCaseId', async (req, res) => {
    try {
        ...
        if (status === 'in_progress') {
            const existingResult = await (0, database_1.query)('SELECT executed_at FROM test_results WHERE test_run_id = $1 AND test_case_id = $2', [id, testCaseId]);
            if (existingResult.rows.length > 0 && !existingResult.rows[0].executed_at) {
                executedAt = new Date();
            }
        }
        const result = await (0, database_1.query)(`UPDATE test_results 
       SET status = $1, notes = $2, duration = $3, executed_by = $4, executed_at = COALESCE($5, executed_at)
       WHERE test_run_id = $6 AND test_case_id = $7 RETURNING *`, [status, notes, duration, '00000000-0000-0000-0000-000000000001', executedAt, id, testCaseId]);
        if (result.rows.length === 0) {
